...
class table_1hand_comments_ext extends discuz_table
{
	public function __construct() {

		$this->_table = '1hand_comments_ext';
		$this->_pk    = '';
	...
	public function fetch_last_by_uid($uid) {
		return DB::fetch_all("SELECT * FROM %t WHERE uid=%d and onlinedate<current_date() order by onlinedate desc limit 0,1", array($this->_table, $uid));
	}

	public function fetch_today_by_uid($uid) {
		return DB::fetch_all("SELECT * FROM %t WHERE uid=%d and onlinedate=current_date() ", array($this->_table, $uid));
	}
	...
	public function update_time1($uid,$otime) {
		return DB::query("update %t set time1=%s where"
		." uid=%d and onlinedate=current_date()", array($this->_table, $otime,$uid));
	}
	public function update_time2($uid,$otime) {
		return DB::query("update %t set time2=%s where"
		." uid=%d and onlinedate=current_date()", array($this->_table, $otime,$uid));
	}
	public function update_time3($uid,$otime) {
		return DB::query("update %t set time3=%s where"
		." uid=%d and onlinedate=current_date()", array($this->_table, $otime,$uid));
	}
	public function update_time4($uid,$otime) {
		return DB::query("update %t set time4=%s where"
		." uid=%d and onlinedate=current_date()", array($this->_table, $otime,$uid));
	}
	...
	public function update($uid,$lastlogin,$contiune) {
		//$sql = sprintf ("UPDATE %t set lastlogin= '%s',contiunelogin = %d where uid=%d",array($this->_table,$lastlogin,$contiune,$uid));
		//echo $sql;
		//DB::query($sql);
		DB::query("UPDATE %t set lastlogin= %s,contiunelogin = %d where uid=%d",array($this->_table,$lastlogin,$contiune,$uid));
	}
