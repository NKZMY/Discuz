...
//查询APP频道下面所有的频道列表,连closed一起传
$sql = 'select a.catid,a.upid,a.catname,a.articles,a.allowcomment,a.displayorder,a.uid,a.username,a.dateline, '
       .' a.closed,a.description,a.lastpublish,b.vcname,b.parameter,b.subtitle,b.logo,b.color,b.smalllogo,b.listlogo, '
       .' b.candelete from '.DB::table('portal_category').' as a '
       .' inner join '. DB::table('1hand_category').' as b on a.catid=b.catid where a.upid='.$upid
       .' order by a.displayorder';//.$_GET['upid']; 在这里可以写死upid
$query = DB::QUERY($sql);
$data = array();
			while($row = DB::fetch($query)) {
				$data[] = cellapp_core::getvalues($row, array('catid', 'upid', 'catname', 'articles', 'allowcomment', 'displayorder', 'notinheritedarticle', 
'notinheritedblock', 'domain', 'url', 'uid', 'username', 'dateline', 'closed', 'shownav', 'description', 'lastpublish',
'vcname','parameter','subtitle','logo','color','smalllogo','listlogo','candelete'));
			}
//查询aid＝5的所有相关文章  aid=5是一篇专门用于存贮			
//catid=0代表全局置顶

$sql = 'select a.aid,0 as catid,a.bid,a.uid,a.username,a.title,a.highlight,a.author,a.from,a.fromurl,a.url,a.summary,'
    ...
    $bouns1 = 50;
      ...
      $message = 		 iconv("gb2312","UTF-8",'连续登陆奖励 ').$bouns. iconv("gb2312","UTF-8",'刀 '); 
		...
		if (count($row)>0){
		 ...
		 $variable = array(
		    'onlineinterval' => '20.0',  //在线心跳时间间隔，以秒为单位
				'data' => $data,
				'datafirst' => $datafirst,
				'message' => $message		);
