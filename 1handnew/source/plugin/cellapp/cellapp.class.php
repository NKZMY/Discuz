...
class cellapp_core {
	...
	function result($result) {
		...
		$result = cellapp_core::format($result);
		echo cellapp_core::json($result);
		exit;
	}
		...
		switch (gettype($result)) {
			case 'array':
				foreach($result as $_k => $_v) {
					$result[$_k] = cellapp_core::format($_v);
				}
				break;
			case 'boolean':
			case 'integer':
		...
		foreach($variables as $key => $value) {
			foreach($keys as $k) {
				if($k{0} == '/' && preg_match($k, $key) || $key == $k) {
					if($subkeys) {
						$return[$key] = cellapp_core::getvalues($value, $subkeys);
					} else {
						if(!empty($value) || !empty($_GET['debug']) || (is_numeric($value) && intval($value) === 0 )) {
							$return[$key] = is_array($value) ? cellapp_core::arraystring($value) : (string)$value;
						}
					}
				}
			}
	...
	function arraystring($array) {
		...
		foreach($array as $k => $v) {
			$array[$k] = is_array($v) ? cellapp_core::arraystring($v) : (string)$v;
		}
		return $array;
	}
...
class base_plugin_cellapp {
	...
	function common() {
		...
		if(!$_G['setting']['mobile']['allowmobile']) {
			cellapp_core::result(array('error' => 'mobile_is_closed'));
		}
		if(!empty($_GET['tpp'])) {
...
class base_plugin_cellapp_forum extends base_plugin_cellapp {
...
class base_plugin_cellapp_misc extends base_plugin_cellapp {
...
class plugin_cellapp extends base_plugin_cellapp {}
class plugin_cellapp_forum extends base_plugin_cellapp_forum {}
class plugin_cellapp_misc extends base_plugin_cellapp_misc {}

class mobileplugin_cellapp extends base_plugin_cellapp {
...
class mobileplugin_cellapp_forum extends base_plugin_cellapp_forum {}
class mobileplugin_cellapp_misc extends base_plugin_cellapp_misc {}

class plugin_cellapp_connect extends plugin_cellapp {
	...
	function login_mobile_message($param) {
		if(substr($_GET['referer'], 0, 7) == 'Mobile_') {
			...
			cellapp_core::result(cellapp_core::variable($param));
		}
	}
