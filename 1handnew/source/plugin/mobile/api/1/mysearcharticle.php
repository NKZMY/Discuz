...
class mobile_api {
	...
	function output() {
		...
		foreach($keyword as $key){
			$test[$number] =  mysql_query("SELECT  aid FROM `pre_portal_article_title` where title like '%$key%'");
			$number++;
			}
		...
		foreach($lastresult as $article_id){
			$testtwo[$numbertwo] =  mysql_query("SELECT  * FROM `pre_portal_article_title` where aid = '$article_id'");
			$numbertwo++;
			}
